...
| M-1 | Medium | 📝 Documented | Swap leg still places the account master pubKey in `swapPubKey` (swaps are linkable). Documented at the source (`pool.ts buildSwap`); the per-swap stealth-key fix is a scoped follow-up (deep per-note-key refactor + dedicated e2e) to keep this fix set atomic. Transfers/unshields are unaffected. |
